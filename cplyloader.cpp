...
{
    printf("Loading %s...\n",filename);
    char* pch = strstr(filename,".ply"); // to check whether the filename have ".ply" extension
    ...
    if (pch != NULL)
    {
       ...
       if (file)
       {
            ...
            // read vertices
            i =0;
            for (iterator = 0; iterator < this->m_totalConnectedPoints; iterator++)//3488
            {
                char tmp[1];
...
//               sscanf(buffer,"%f %f %f %c %f %f %f", &mp_vertexXYZ[i], &mp_vertexXYZ[i+1], &mp_vertexXYZ[i+2],
//                       tmp,
//                       &mp_vertexRGB[i], &mp_vertexRGB[i+1],  &mp_vertexRGB[i+2]);

                sscanf(buffer,"%f %f %f %f %f %f", &mp_vertexXYZ[i], &mp_vertexXYZ[i+1], &mp_vertexXYZ[i+2], &mp_vertexRGB[i], &mp_vertexRGB[i+1],  &mp_vertexRGB[i+2]);
            ...
            // read faces
            i =0;
            for (iterator = 0; iterator < this->m_totalFaces; iterator++)//6920
            {
                fgets(buffer,300,file);
                    ...
                    if (buffer[0] == '3')
                    {
                        ...
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex1]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex1+1]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex1+2]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex2]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex2+1]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex2+2]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex3]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex3+1]);
                        m_ModelData.vecFaceTriangleColors.push_back( mp_vertexRGB[3*vertex3+2]);
